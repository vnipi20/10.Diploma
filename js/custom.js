...
calculation.addEventListener('click', function () {
    let getRangeHeight = heightValue.textContent; /*--получение значение высоты помещение--*/
    let getReflectionCeiling = reflectionCeilingValue.textContent; /*--получение значение К-потолка--*/
    let getReflectionWalls = reflectionWallsValue.textContent; /*--получение значение К-стен--*/
    let getReflectionFloor = reflectionFloorValue.textContent; /*--получение значение К-пола--*/
    let getHeightPlane = heightPlaneValue.textContent; /*--получение значение высоты расчетной плоскости--*/
    let getReserve = reserveValue.textContent; /*--получение значение К-запаса--*/
    /*--формулы расчета освещенности--*/
...
/*--переключение подсветки имени светильника--*/
...
for (let i = 0, len = lampSubtype.length; i < len; i++) {
    lampSubtype[i].addEventListener('click', function () {
        ...
        $(document).ready(function () {
            ...
            function mainDescriptionSlider(date) {
                let mainDescriptionSlider = date.map((el, item) => {
                    return (`
                ...
                $('.more-description-wrap').replaceWith(mainDescriptionSlider);
            }

            /*--главные характеристики светильника--описание--*/
            function mainDescription(date) {
                let mainDescription = date.map((el, item) => {
                    return (`
                  <div class="column-1">
                  <div>
                    <h4>Установка:</h4>
                ...
                $('.column-1').replaceWith(mainDescription);
            }
...
let btnDescriptionOpenClose = document.getElementById('btnDescriptionOpenClose');

btnDescriptionOpenClose.addEventListener('click', function () {

    let hideDiscription = document.getElementById('mainDescription');
